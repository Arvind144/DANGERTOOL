...
def main():
    ...
    while True:
        ...
        print(f"{Fore.YELLOW}Press 'q' to quit{Style.RESET_ALL}")
        choice = input(f"{Fore.GREEN}Your choice (1/2/3 or q): {Style.RESET_ALL}").strip().lower()
        if choice == 'q':
            print(f"{Fore.CYAN}👋 Quitting the script. Goodbye!{Style.RESET_ALL}")
            return
        elif choice == '1':
            dat_file = fresh_start(dat_files)
            changes_made = []
