...
def get_directories():
    ...
    Return the required directories with predefined paths:
      ...
      5. Repack (destination folder): /storage/emulated/0/FILES_OBB/REPACK_OBB/REPACKGUN
      6. Skin Index file path: /storage/emulated/0/FILES_OBB/TXT/skin_index.txt
    """
    dirs = {
        ...
        "repack": "/storage/emulated/0/FILES_OBB/REPACK_OBB/REPACKGUN",
...
def copy_files_to_repack_mod(mod_type, src_dir, repack_folder, condition_hexes, file_modtype_map):
    ...
    for file_name in os.listdir(src_dir):
        ...
        if not os.path.isfile(src_file_path):
            continue
        # Do not hardcode any filename; always use the original file name.
        dest_file_name = file_name
        dest_file_path = os.path.join(repack_folder, dest_file_name)
        
        # If file already exists, check if it already contains one of the condition hexes.
        if os.path.exists(dest_file_path):
            try:
                with open(dest_file_path, "rb") as f:
                    dest_data = f.read().hex()
                if any(hex_val in dest_data for hex_val in condition_hexes):
                    file_modtype_map[dest_file_name] = mod_type
                    continue
            except Exception:
                pass
...
def mod_hit_effect_file(file_path, source_hex, target_hex, source_gun, target_gun):
    ...
    try:
        with open(file_path, "rb") as f:
            data = f.read().hex()
        # Look for the source level 5 hex as a continuous block.
...
def mod_icon_file(file_path, source_hex, target_hex, source_index_hex, target_index_hex, source_gun, target_gun):
    ...
    try:
        with open(file_path, "rb") as f:
            data = f.read().hex()
        # Ensure the file contains exactly two occurrences of source_hex.
        occurrences = len(re.findall(re.escape(source_hex), data))
        if occurrences != 2:
            return None
        intervals = []
...
def bulk_modding(guns, dirs, skin_index_dict):
    ...
    # Process each pair without clearing repack folder between pairs.
    for source_id, target_id in pairs:
        ...
        # Copy files from each mod directory using original filenames.
    ...
    # Clean up repack folder: remove files not referenced in the changelog.
    changelog_files = set()
    ...
    if mod_counters["Gun Skins"] == 0:
        print(Fore.YELLOW + "⚠️ No Gun Skins modding was applied.")
    if mod_counters["Hit Effect"] == 0:
        print(Fore.YELLOW + "⚠️ No Hit Effect modding was applied.")
    if mod_counters["Lootbox"] == 0:
        print(Fore.YELLOW + "⚠️ No Lootbox modding was applied.")
    if mod_counters["Icon"] == 0:
        print(Fore.YELLOW + "⚠️ No Icon modding was applied.")
...
def normal_modding(guns, dirs, skin_index_dict):
    ...
    while True:
        ...
        print(Fore.GREEN + "\n👋 Normal modding complete. Returning to mode selection...\n")
        break
